...
impl UpgradeProcess {
	...
	pub fn run(self, target: RunTo) -> UpgradeProcessInfo {
		...
		let result = tokio::spawn(async move {
			...
			self.input.update()?;
			out_tx.send(UpgradeState::CheckingUpgrades).unwrap();
			let out = self.input.dry_build()?;
